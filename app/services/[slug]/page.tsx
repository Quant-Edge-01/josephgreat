...
export default async function IndustryPage({ params }: Props) {
  ...
  return <ServiceDocument title={page.title} intro={page.intro} path={path}>
    ...
    <DocumentSection title="The relevant evidence"><Image src={proof.images[0].src} alt={proof.images[0].alt} width={800} height={800} className="max-h-80 w-full max-w-lg object-contain object-left" /><p>{page.proof}</p><Link className="inline-block py-2 text-neon underline underline-offset-4" href={`/works/${page.proofSlug}`}>Read the {workBySlug(page.proofSlug)?.title} case and see the screenshots →</Link></DocumentSection>
    ...
    <DocumentSection title="Price and exclusions"><p>Monthly work is CAD $700–$1,000 depending on the written scope. Ad spend is separate. A full website, unlimited revisions, third-party subscriptions and additional filming or travel costs are not automatically included.</p><Link href="/affordable-marketing-toronto" className="inline-block py-2 text-neon underline">Pricing and scope →</Link></DocumentSection>
